...
{
public:
    // Encrypt a std::u16string. The output (second argument) is really an array of bytes, but
    // we're passing it back as a std::string.
    static bool encryptString16(std::u16string plaintext, std::string* ciphertext);
    ...
    // data into a string.
    static bool decryptString16(std::string ciphertext, std::u16string* plaintext);

    // Encrypt a string.
    static bool encryptString(std::string plaintext, std::string* ciphertext);
    ...
    // string.
    static bool decryptString(std::string ciphertext, std::string* plaintext);
